...
class Beta(object):
    ...
    def plot_distribution(self, df1_avg, df2_avg, tag, df1_name, df2_name):
        ...
        #plotting function
        ax.plot(5*x, stats.beta.pdf(x, df1_a, df1_b),'r-', lw=5, alpha=0.5, label="{0} readers ratings of {1}".format(df1_name, df2_name))
        ax.plot(5*x, stats.beta.pdf(x, df2_a, df2_b),'b-', lw=5, alpha=0.5, label="{0} readers ratings of {1}".format(df2_name, df1_name))
        ax.legend()

        ax1.hist(5 * df1_avg[tag], color='r', bins = 20, rwidth = 0.75, label= "Mean User Raitings")
        ax2.hist(5 * df2_avg[tag], color = 'b',bins = 20, rwidth = 0.75, label= "Mean User Raitings")
